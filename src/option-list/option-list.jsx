   ...
   lookup = (event) => {
     ...
     if (
       this.matchTermBuilder.match(/^(.)\1+$/) && // regex matching a repeating char
         // and make sure the repeating char matches the first char
         this.matchTermBuilder
           .charAt(0)
           .toLowerCase() === firstListItem.charAt(0).toLowerCase()
     ) {
       ...
       for (let i = 0; i < this.matchStore.length; i++) {
         ...
         if (
         // comparision only on length of current match term
           listItem
             .substring(0, this.matchTermBuilder.length)
             .toLowerCase() ===
